...
from modules.core.rabbitmq.messages.jira_tracker.get_worklogs_response import GetWorklogsResponse
...
class GetWorklogsRequestHandler(RpcBaseHandler):
    ...
    def execute(self, payload) -> str:
        ...
        day_statistics: list[WorklogDay] = []

        startDay = datetime(year, month, 1)
        end_month = month+1
        end_year = year
        ...
        endDay = datetime(end_year, end_month, 1)
        ...
        for day in range(1, num_days+1):
            ...
            day_statistics.append(WorklogDay(targetDay, sum_timespent))
        jira.close()
        response = GetWorklogsResponse(day_statistics)
        self.logger_service.send_log(DEBUG_LOG_LEVEL, self.TAG, 'Disconnected to jira for get time...')

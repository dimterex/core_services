...
IPTV_SOURCES_TABLE_NAME = 'iptv_epg_sources'
IPTV_SOURCES_VALUE_COLUMN_NAME = 'value'


class IptvEpgSourcesTable(BaseTable):
    ...
    def get_sources(self) -> list[IptvEpgSourceModel]:
        data = self.get_data()
        models = []
        for row in data:
            models.append(IptvEpgSourceModel(row[0], row[1]))
        return models

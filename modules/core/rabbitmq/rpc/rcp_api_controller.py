...
class RpcApiController:
    ...
    def received(self, message: str) -> str:
        if message is None:
            return self.generate_exception_response('Message is None')
        try:
            ...
            type_handlers = self.handlers[type]

            return type_handlers.execute(payload)
        except Exception as e:
            error_message = f'Internal exception. \n Error: {e} \n Message: {message}'
            self.logger_service.send_log(ERROR_LOG_LEVEL, self.TAG, error_message)
            return self.generate_exception_response(error_message)

    def generate_exception_response(self, message: str) -> str:
        return StatusResponse(ERROR_STATUS_CODE, message).to_json()
